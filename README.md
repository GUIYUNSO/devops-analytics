...
**知识图谱构建**：
- 实体：开发者、模块、合并请求、提交、议题、缺陷、部署
- 关系：编写、提交、评审、归属、修改、解决、引入、导致
- 支持查询：给定实体找所有关联实体、两个实体之间的路径、模块的缺陷数量
...
```
缺陷 → 哪次部署引入的？→ 哪个合并请求在那次部署里？→ 哪个提交改了相关代码？→ 根因分类
```

根因分类：代码缺陷 / 配置变更 / 依赖问题 / 基础设施 / 流程缺失
...
输出：支付模块风险分=0.78（高），主要因子：变更频率=0.85、缺陷率=3.2、单点依赖=1
```
...
输出：缺陷 +50% 环比，主要集中在支付模块，根因是 3 个大合并请求缺少评审
```
...
`~/.eia/memory.md`，纯文本仅追加文件。只记录分析摘要（如「支付模块 30 天 3 个缺陷」），不记录代码内容。你可以随时查看、编辑或删除。
